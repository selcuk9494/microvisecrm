...
export default function LicensesCsv() {
  ...
  async function load() {
    ...
    try {
      ...
      const [ls, cs, vs] = await Promise.all([
        apiFetch(`/licenses?${qs.toString()}`),
        apiFetch(`/customers?page=1&pageSize=999`),
        apiFetch(`/software-vendors`)
      ]);
      setRows(ls.data || []);
  ...
  function exportCsv() {
    const lines = [
      ["Yazılım Firması","Müşteri","Aktivasyon","Bitiş","Durum","Cihaz"],
      ...rows.map((r) => [
        (vendors.find((v:any)=>v.id===r.vendorId)?.name) || "",
  ...
  return (
    <div>
      <div className="mb-4 flex items-center justify-between">
        <h1 className="text-2xl font-semibold">Lisans CSV</h1>
      ...
      <div className="card overflow-hidden">
        <table className="w-full table-auto text-sm">
          <thead className="bg-slate-100 text-left">
            <tr>
              <th className="px-3 py-2">Yazılım Firması</th>
          ...
          <tbody>
            {rows.map((r)=> (
              <tr key={r.id} className="border-t">
                <td className="px-3 py-2">{vendors.find((v:any)=>v.id===r.vendorId)?.name || ""}</td>
